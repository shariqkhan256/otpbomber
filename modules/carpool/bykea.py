...
async def bykea(phone, client, out):
    name = 'bykea'
    domain = 'bykea.com'
    frequent_rate_limit = False
    headers = {'User-Agent': random.choice(ua['browsers']['chrome']), 'Content-Type': 'application/json'}
    json_data = {'country_code': '92', 'phone_number': phone[-10:]}
    try:
        response = await client.post('https://api.bykea.com/v1/user/otp/send', headers=headers, json=json_data)
